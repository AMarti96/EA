  ...
  describe('bug fixes', function() {
    ...
    it('creates new array when initializing from existing doc (gh-4449)', function(done) {
      ...
      User.create(val, function(error, val) {
        assert.ifError(error);
        val.users.push({ title: 'Groceries' });
        val.save(function(error) {
          assert.ifError(error);
          User.findById(val, function(error, val) {
            assert.ifError(error);
            assert.deepEqual(val.toObject().users, [{ title: 'Groceries' }]);
            var u2 = new User();
            val.users = u2.users;
            val.users.push({ title: 'Cook' });
            val.save(function(error) {
              assert.ifError(error);
              User.findById(val, function(error, val) {
                assert.ifError(error);
                assert.equal(val.users.length, 1);
                assert.equal(val.users[0].title, 'Cook');
                done();
              });
            });
          });
